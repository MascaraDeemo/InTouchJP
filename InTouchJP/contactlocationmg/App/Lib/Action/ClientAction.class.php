...
/************************************************公共方法结束***************************************************************/
    ...
    public function addMessage(){
        ...
        $data['fusername'] = $_REQUEST['fusername'];
        $data['attach'] = $_REQUEST['attach'];
        $data['ndate'] = date('Y-m-d H:i:s');
        $data['attachname'] = $_REQUEST['attachname'];
        $model = M("Message");
        $model->add($data);
        $this->renderJsonpObj($data);
    }

    public function listMyMessage(){
        $uid = $_REQUEST['uid'];
        $fid = $_REQUEST['fid'];
        $db = M();
        $list = $db->query("select * from t_message where 1=1 and (uid='$uid' or fid='$uid') and (uid='$fid' or fid='$fid')");
        $this->renderJsonpObj($list);
    }

    public function listToMyMessage(){
        $uid = $_REQUEST['uid'];
        $model = M('Message');
        $cdt['fid'] = $uid;
        $this->renderJsonpObj($model->where($cdt)->select());
    }

    public function listToMyMessage2(){
        $uid = $_REQUEST['uid'];
        $model = M('Replay');
        $cdt['fid'] = $uid;
        $this->renderJsonpObj($model->where($cdt)->select());
